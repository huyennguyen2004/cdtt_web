...
class CreateBannerTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('banner', function (Blueprint $table) {
            ...
            $table->string('link', 1000)->nullable(); // 'link' with varchar(1000) and NULL
            $table->string('image', 1000)->nullable(false); // 'image' with varchar(1000) and NOT NULL
            $table->unsignedInteger('sort_order')->default(0); // 'sort_order' with INT DEFAULT 0, UNSIGNED
            $table->enum('position', ['slideshow', 'ads'])->default('slideshow'); // 'position' with ENUM and DEFAULT 'slideshow'
