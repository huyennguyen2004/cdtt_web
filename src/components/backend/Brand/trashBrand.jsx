...
const TrashBrand = () => {
  ...
  const [trashedbrands, setTrashedbrands] = useState([
    { id: 1, name: 'Sản phẩm A', image: 'https://via.placeholder.com/100', brand: 'Danh mục 1', brand: 'Thương hiệu 1' },
    { id: 2, name: 'Sản phẩm B', image: 'https://via.placeholder.com/100', brand: 'Danh mục 2', brand: 'Thương hiệu 2' },
  ...
  // Hàm quay lại
  const handleGoBack = () => {
    navigate('/admin/brand'); // Quay lại trang sản phẩm
  };
      ...
      <table className="min-w-full bg-white">
        ...
        <tbody>
          {trashedbrands.map((brand) => (
            <tr key={brand.id}>
              <td className="px-6 py-4 border-b border-gray-200">{brand.id}</td>
              <td className="px-6 py-4 border-b border-gray-200">{brand.name}</td>
              <td className="px-6 py-4 border-b border-gray-200">{brand.slug}</td>
              <td className="px-6 py-4 border-b border-gray-200 flex space-x-2">
                <button onClick={() => handleRestore(brand.id)} className="text-green-600 hover:underline flex items-center">
                  <FaUndo className="mr-1" /> 
                </button>
                <button onClick={() => handleDestroy(brand.id)} className="text-red-600 hover:underline flex items-center">
